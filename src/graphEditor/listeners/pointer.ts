...
import {
  fitCanvasToDisplaySize, // used indirectly by resize module; left here for symmetry
  getCanvasPoint,
  hitTestNode,
  hitTestPort,
  hitTestConnection,
...
export const makePointerMove = (state: GraphState) => (evt: PointerEvent) => {
  ...
  if (state.marquee.active) {
    ...
    for (const n of state.nodes) if (intersectsNode(x, y, width, height, n)) nodeIds.add(n.id);

    const connIds = new Set<string>();
    for (const c of state.connections) if (connectionIntersectsRect(state, c, x, y, width, height)) connIds.add(c.id);
    ...
    if (hasNodes && !hasConns) {
      state.selectedConnectionIDs.clear();
      state.selectedIDs = additive ? new Set<string>([...prevNodeSel, ...nodeIds]) : (nodeIds as Set<NodeID>);
    } else if (hasConns && !hasNodes) {
      state.selectedIDs.clear();
      state.selectedConnectionIDs = additive ? new Set<string>([...prevConnSel, ...connIds]) : connIds;
    } else {
      if (state.selectionMode === "connection") {
        state.selectedIDs.clear();
        state.selectedConnectionIDs = additive ? new Set<string>([...prevConnSel, ...connIds]) : connIds;
  ...
  if (state.hoverPortID) {
    ...
    if (state.hoverConnectionID) {
      if (state.hoverID !== null) { state.hoverID = null; needRender = true; }
    } else if (newHoverNode !== state.hoverID) { state.hoverID = newHoverNode; needRender = true; }
  }
