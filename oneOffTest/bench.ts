...
type CSR = { offsets: Int32Array; children: Int32Array; degree: Int32Array };
...
const N = arg("n", 300_000);         // total nodes
...
const now = (): number => {
  if (isNode && (process as any).hrtime?.bigint) {
    return Number((process as any).hrtime.bigint()) / 1e6;
  }
  return performance.now();
};
const doGC = () => (globalThis as any).gc?.();
...
function buildTopology(n: number, maxChildren: number, seed: number) {
  if (n <= 0) throw new Error("buildTopology: n must be > 0");
  if (maxChildren <= 0) throw new Error("buildTopology: bf must be > 0");
...
// ---------- topology validation ----------
function validateTopology(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array; }) {
  ...
  while (sp) {
    const i = stack[--sp];
    if (i < 0 || i >= n) throw new Error(`validate: out-of-bounds node index ${i}`);
    if (seen[i]) continue;
    seen[i] = 1; visited++;
    ...
    for (let c = firstChild[i], guard = 0; c !== SENTINEL; c = nextSibling[c]) {
      if (++guard > n) throw new Error(`validate: sibling chain cycle detected at parent ${i}`);
      if (c < 0 || c >= n) throw new Error(`validate: child OOB: parent ${i} -> ${c}`);
      if (c === prev) throw new Error(`validate: sibling self/loop at ${c}`);
      prev = c;
      stack[sp++] = c;
    }
  }
  ...
  if (visited !== n) {
    let firstMissing = -1;
    for (let i = 0; i < n; i++) if (!seen[i]) { firstMissing = i; break; }
    throw new Error(`validate: reachable=${visited}/${n}; example missing index=${firstMissing}`);
  }
}

// ---------- orders ----------
function orderDFS(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array; }): Int32Array {
  const { firstChild, nextSibling } = topo;
  ...
  while (sp) {
    ...
    const tmp: number[] = [];
    for (let c = firstChild[i]; c !== SENTINEL; c = nextSibling[c]) tmp.push(c);
    for (let j = tmp.length - 1; j >= 0; j--) stack[sp++] = tmp[j];
  }
  if (k !== n) throw new Error(`orderDFS: produced ${k}/${n} nodes`);
  return order;
}

function orderBFS(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array; }): Int32Array {
  ...
  while (head < tail) {
    const i = q[head++];
    order[k++] = i;
    for (let c = firstChild[i]; c !== SENTINEL; c = nextSibling[c]) q[tail++] = c;
  }
  if (k !== n) throw new Error(`orderBFS: only enqueued ${k}/${n} nodes — topology might be disconnected`);
  return order;
}

// ---------- CSR builder (1) ----------
function buildCSR(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }): CSR {
  const { firstChild, nextSibling } = topo;
  const degree = new Int32Array(n);
  for (let i = 0; i < n; i++) {
    for (let c = firstChild[i]; c !== SENTINEL; c = nextSibling[c]) degree[i]++;
  }
...
function computeSubtreeSizes(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }): Int32Array {
  const { firstChild, nextSibling } = topo;
  ...
  while (sp) {
    ...
    if (iter[sp - 1] === 0) {
      iter[sp - 1] = 1;
      for (let c = firstChild[i]; c !== SENTINEL; c = nextSibling[c]) {
        stack[sp] = c; iter[sp++] = 0;
      }
    } else {
      // exit
      let s = 1;
      for (let c = firstChild[i]; c !== SENTINEL; c = nextSibling[c]) s += sz[c];
      sz[i] = s;
      sp--;
    }
  }
...
// ---------- AoS builders ----------
function buildAoS(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }, seed: number) {
  const { firstChild, nextSibling } = topo;
...
function buildAoSPacked(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }, seed: number, order: Int32Array) {
  const { firstChild, nextSibling } = topo;

  const newIdxOfOld = new Int32Array(n);
  for (let newI = 0; newI < n; newI++) newIdxOfOld[order[newI]] = newI;
...
// ---------- (2) Preorder pack + CSR + subtreeSize ----------
type PreorderPack = {
...
function buildPreorderCSRAndSubtree(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }, seed: number): PreorderPack {
  ...
  for (let oldP = 0; oldP < n; oldP++) {
    let d = 0;
    for (let c = topo.firstChild[oldP]; c !== SENTINEL; c = topo.nextSibling[c]) d++;
...
function buildBlockedDFSPack(n: number, topo: { firstChild: Int32Array; nextSibling: Int32Array }, seed: number, tileBytes: number) {
  ...
  for (let t = 0; t < n; t += nodesPerTile) {
    ...
    for (let i = t; i < end; i++) blockedOrder[k++] = order[i];
  }
  return buildAoSPacked(n, topo, seed ^ 0xB10C, blockedOrder);
}

// ---------- Objects from AoS ----------
function buildObjectsFromAoS(I32: Int32Array, U32: Uint32Array): ObjNode[] {
  const n = I32.length / LANES;
  const nodes: ObjNode[] = new Array(n);
  for (let i = 0; i < n; i++) {
    const b = i * LANES;
    nodes[i] = {
      index: I32[b + 0],
...
function dfsAoS_NoGate(I32: Int32Array, U32: Uint32Array, rootIdx: number) {
  const n = I32.length / LANES;
  let checksum = 0 >>> 0;
  let visited = 0;
  ...
  const push = (v: number) => {
    if (v !== SENTINEL) {
      if (v < 0 || v >= n) throw new Error(`dfsAoS: pushed OOB index ${v}`);
      if (sp >= stack.length) {
        if (stack.length >= n) throw new Error(`dfsAoS: stack growth would exceed n=${n}`);
        const grow = new Int32Array(Math.min(stack.length << 1, n));
        grow.set(stack); stack = grow;
      }
      stack[sp++] = v;
    }
  };
  ...
  let i = rootIdx;
  if (i < 0 || i >= n) throw new Error(`dfsAoS: root OOB ${i}`);

  let iters = 0;
  const ITER_CAP = 2 * n;
  while (true) {
    if (++iters > ITER_CAP) throw new Error(`dfsAoS: iteration cap exceeded (${iters} > ${ITER_CAP})`);
    ...
    if (child !== SENTINEL && (child < 0 || child >= n)) throw new Error(`dfsAoS: child OOB at ${i} -> ${child}`);
    if (sib   !== SENTINEL && (sib   < 0 || sib   >= n)) throw new Error(`dfsAoS: sibling OOB at ${i} -> ${sib}`);

    checksum = toU32(checksum + ((idx ^ child ^ sib ^ flags) >>> 0));
    visited++;

    if (child !== SENTINEL) {
      if (sib !== SENTINEL) push(sib);
      i = child;
      continue;
    }

    if (sib !== SENTINEL) { i = sib; continue; }
    if (sp === 0) break;
    i = stack[--sp];
  }
...
function dfsObj_NoGate(nodes: ObjNode[], rootIdx: number) {
  ...
  const push = (v: number) => {
    if (v !== SENTINEL) {
      if (v < 0 || v >= n) throw new Error(`dfsObj: pushed OOB index ${v}`);
      if (sp >= stack.length) {
        if (stack.length >= n) throw new Error(`dfsObj: stack growth would exceed n=${n}`);
        const grow = new Int32Array(Math.min(stack.length << 1, n));
        grow.set(stack); stack = grow;
      }
      stack[sp++] = v;
  ...
  let i = rootIdx;
  if (i < 0 || i >= n) throw new Error(`dfsObj: root OOB ${i}`);

  let iters = 0;
  const ITER_CAP = 2 * n;
  while (true) {
    if (++iters > ITER_CAP) throw new Error(`dfsObj: iteration cap exceeded (${iters} > ${ITER_CAP})`);

    const node = nodes[i];
    const child = node.childIndex;
    const sib = node.siblingIndex;

    if (child !== SENTINEL && (child < 0 || child >= n)) throw new Error(`dfsObj: child OOB at ${i} -> ${child}`);
    if (sib   !== SENTINEL && (sib   < 0 || sib   >= n)) throw new Error(`dfsObj: sibling OOB at ${i} -> ${sib}`);
...
function dfsCSR(I32: Int32Array, U32: Uint32Array, csr: CSR, root = 0) {
  const n = I32.length / LANES;
  ...
  let sp = 0;
  const push = (node: number, cur: number) => {
    if (sp >= stackNode.length) {
      if (stackNode.length >= n) throw new Error(`dfsCSR: stack growth would exceed n=${n}`);
      const growN = new Int32Array(Math.min(stackNode.length << 1, n));
      const growC = new Int32Array(growN.length);
      growN.set(stackNode); growC.set(stackCur);
      stackNode = growN; stackCur = growC;
    }
    stackNode[sp] = node; stackCur[sp] = cur; sp++;
  };
  ...
  while (true) {
    if (++iters > ITER_CAP) throw new Error(`dfsCSR: iteration cap exceeded (${iters} > ${ITER_CAP})`);
...
function dfsPreorderCSR(I32: Int32Array, U32: Uint32Array, csr: CSR, _subtree: Int32Array, root = 0) {
...
function bench(label: string, fn: () => { checksum: number; visited: number }, trials: number) {
  // warmup
  fn(); fn();
...
const nsPerVisited = (ms: number, visited: number) => (ms * 1e6) / Math.max(1, visited);
