# Overview
The tree module connects 2 ECS component stores into single-root tree. `Tree` bootstraps paired data and node stores, creates a protected root entity, keeps a stable depth-first traversal order, and offers helpers to reparent or remove subtrees. `TransformTree` extends this by updating transform components so entities keep their world-space pose when their parent changes.

## API
### Shared helpers
- `isTreeStore(store)` – checks that a store’s meta matches the expected parent, child, and sibling fields.
- `nodeColumns(store)` – narrows the untyped column map to the tree link columns.
...
### `Tree`
- `constructor(world, dataMeta, nodeMeta, rootData)` – registers the metas, allocates the root entity, seeds row 0 in both stores, protects the root, and registers the tree with the world.
...
- `dispose()` – unregisters the tree and releases the root’s protection so the world can tear down cleanly.
...
## Testing Plan
### Module Helpers
- `isTreeStore` accepts a store with all required link fields and rejects stores with any field missing or incorrectly typed.
...
- Destroying entities through the world triggers the tree callback registered during construction.
...
- Calling `dispose` unregisters the tree and unprotects the root so the world can delete it later.
